    ...
    if st.button("Generar factura PDF"):
        ...
        for item in pdf_data:
            pdf.multi_cell(0, 8,
                ...
                f"Personas: {item['personas']}\n"
                f"Traslados incluidos: {item['traslado_cost'] and 'Sí' or 'No'}\n"
                f"Vehículo: {item['vehiculo']} (¥{item['vehiculo_costo']:,})\n"
                f"Base: ¥{item['base']:,} | Extra pax: ¥{item['extra']:,} | Traslados: ¥{item['traslado_cost']:,} | Total Día: ¥{item['total']:,}\n", border=0)
            pdf.ln(2)
